...
wb = xlrd.open_workbook('/media/ryoji/ボリューム1/simulation_data/rnn_simulation_step1.xlsx')
sheet = wb.sheet_by_name('Sheet1')

#エクセルから値を抽出して配列に格納
values = [[int(sheet.cell_value(row, 4))] for row in range(5, sheet.nrows)] #特徴量
signals = [[int(sheet.cell_value(row, 5))] for row in range(5, sheet.nrows)] #信号データ
...
model_name = 'step1_BATCH' + str(BATCH_SIZE) + 'epoch' + str(EPOCH) + 'LSTM' + str(LSTM_units) + '.h5'
